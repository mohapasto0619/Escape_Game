            ...
            if (userQuery.documents.isNotEmpty()) {
                for (document in userQuery) {
                    ...
                    docRef.get(Source.SERVER).addOnSuccessListener { documentSnapshot ->
                        ...
                        val closed = documentSnapshot.get("closed") as Boolean
                        enigme = Enigme(id.toInt(), name, reponse, state, indice, closed)
    ...
    suspend fun setEnigmeOpen(enigmeTag: String,type: Int) {
        ...
        when (type) {
            0 -> refDoc.update(
                ...
                Log.d("ENIGMA", "setEnigmeOpen: SET TO TRUE  ")
            }.await()
            1 -> refDoc.update(
                mapOf(
                    "closed" to false,
                ...
                Log.d("ENIGMA", "setEnigmeOpen: SET TO FALSE  ")
            }.await()
        }
    }
